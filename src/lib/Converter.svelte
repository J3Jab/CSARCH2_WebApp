<script lang="ts">
    ...
    function binaryToHexadecimal(){
        let fullBin = signBit + combiField + expContinuation + first3digitsDPBCD + last3digitsDPBCD
        let hexa = "0x"
        let cutoff = 0
        let substring_bin = ""
    ...
    function convert(value: string, exponent: number){
        ...
        return signBit + combiField + expContinuation + first3digitsDPBCD + last3digitsDPBCD
            
    }
    
    export function decimalTo32Float(decimal: string, exponent: number) {
        
        return convert(decimal, exponent)
    }

    export function decimalTo32FloatHexadecimal(decimal: string, exponent: number) {
        convert(decimal, exponent)
